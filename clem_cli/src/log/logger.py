...
class Logger:
    ...
    def extract_and_log_code(self, response_text, code_block_delimiter='```'):
        """
        Extracts and logs code blocks from an LLM response.
        
        Args:
            response_text (str): The full response from the LLM
            logger (logging.Logger): Logger instance to use (optional)
            code_block_delimiter (str): Delimiter used by the LLM to mark code blocks (default: ```)
